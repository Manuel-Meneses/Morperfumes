...
// 1. Importamos el carrito lateral que creaste
import { CartDrawer } from "./cart-drawer" 

export function Header() {
  const { itemCount } = useCart()
  const [open, setOpen] = useState(false) // Estado para el menú del celular
  const [isCartOpen, setIsCartOpen] = useState(false) // 2. Estado para el carrito lateral
  ...
  return (
    <>
      <header className="relative w-full border-b border-[#141f36]/20 bg-[#f6f4ed]">
        <div className="container mx-auto flex h-20 items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-4 text-[#141f36]">
            ...
            {/* 3. Botón de Carrito (Reemplazado el Link por un botón que abre el Drawer) */}
            <Button 
              variant="ghost" 
              size="icon" 
              className="relative hover:bg-[#141f36]/5 hover:text-[#c0a062]"
                  ...
                  <nav className="flex flex-col gap-0">
                    ...
                    <button
                      onClick={() => {
                        ...
                        setIsCartOpen(true); // Abre el carrito lateral
                      }}
      ...
      {/* 5. Colocamos el componente del carrito lateral en la base del Header */}
